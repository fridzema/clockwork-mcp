...
describe('Performance Tools', () => {
  ...
  describe('getPerformanceSummary', () => {
    it('returns performance metrics for a request', () => {
      const result = getPerformanceSummary(getStorage(), { requestId: 'req-1' });
  ...
  describe('getTimeline', () => {
    it('returns timeline events', () => {
      const result = getTimeline(getStorage(), { requestId: 'req-1' });
      expect(result).toHaveLength(2);
      expect(result[0].description).toBe('Boot');
    });

    it('returns empty array when no timeline', () => {
      const result = getTimeline(getStorage(), { requestId: 'req-2' });
  ...
  describe('compareRequests', () => {
    it('compares two requests', () => {
      const result = compareRequests(getStorage(), { requestId1: 'req-1', requestId2: 'req-2' });
