...
describe('Request Tools', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      join(testDir, '1705312345-first.json'),
      JSON.stringify({
        id: '1705312345-first',
        type: 'request',
        time: 1705312345,
    ...
    writeFileSync(
      join(testDir, '1705312346-second.json'),
      JSON.stringify({
    ...
    writeFileSync(
      join(testDir, '1705312347-third.json'),
      JSON.stringify({
        id: '1705312347-third',
        type: 'request',
        time: 1705312347,
  ...
  describe('listRequests', () => {
    it('returns all requests', () => {
      const result = listRequests(getStorage(), {});
      expect(result).toHaveLength(3);
    });

    it('filters by method', () => {
      const result = listRequests(getStorage(), { method: 'POST' });
      expect(result).toHaveLength(1);
      expect(result[0].method).toBe('POST');
    });

    it('filters by uri pattern', () => {
      const result = listRequests(getStorage(), { uri: '/api' });
      expect(result).toHaveLength(2);
    });

    it('respects limit', () => {
      const result = listRequests(getStorage(), { limit: 2 });
      expect(result).toHaveLength(2);
    });
  });
  ...
  describe('getRequest', () => {
    it('returns full request data', () => {
      const result = getRequest(getStorage(), '1705312345-first');
    ...
    it('returns null for non-existent request', () => {
      const result = getRequest(getStorage(), 'nonexistent');
  ...
  describe('getLatestRequest', () => {
    it('returns most recent request', () => {
      const result = getLatestRequest(getStorage());
  ...
  describe('searchRequests', () => {
    it('searches by controller', () => {
      const result = searchRequests(getStorage(), { controller: 'UserController' });
      expect(result).toHaveLength(2);
    });

    it('searches by min duration', () => {
      const result = searchRequests(getStorage(), { minDuration: 100 });
      expect(result).toHaveLength(1);
